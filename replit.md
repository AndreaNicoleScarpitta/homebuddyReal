...
- **AI Integration**: Document analysis workflow (GPT-4o) — users upload PDF/text documents, the system extracts text, sends it to the LLM for structured JSON extraction of home issues, and presents detected tasks for review before adding to the user's task list. No chat UI. All extracted attributes are system-scoped (e.g., `roof_material`, `siding_condition`, `hvac_unit_type`) — generic attribute names like `material` or `condition` are never used. Cross-system attribute bleed is prevented by `validateAttributeNamespace` in `server/lib/document-analysis.ts`. Unknown-system attributes are prefixed with `unknown_system_`. Vision API capabilities for photo analysis and circuit panel identification remain.
